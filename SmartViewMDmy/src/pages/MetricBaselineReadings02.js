...
function roundDate(timeStamp, interval){
 ...
 function getUser(userReference,divID) {
  // [START get_document]
...
function Report(props) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = usersRef.onSnapshot((querySnapshot) => {
      const readings = querySnapshot.docs.map( (user) => {
        // const readings =  fetchReadings(user.data())
        ...
        if (user.data().uid) {
          ...
          const getReadings = readingsRef.onSnapshot((querySnapshot) => {
            
            const userReadings= querySnapshot.docs.map( (reading,i) => { 
              const fetchedReading = {
                id: reading.id,
                ...reading.data(),
                user: user.data()
              };
              //allReadings.push(fetchedReading);
              //console.log('len',allReadings.length)
              //console.log(`fetchedReading ${i}`,fetchedReading)
              return fetchedReading
            })
            console.log(`readings ${j++}`,readings)

            setUsers(userReadings)
      ...
      // })
      //console.log('Final Readings',users)
              ...
              {console.log('REaDings',readings)?'':''}
              {
                (
                  readings && readings.length>0 ?
                    (readings
                      .map((reading) =>Object.entries(reading.devices).map((device) => device[0]))
